...
| `config-signals.json` | Reads `metrics` section: primary horizon, partial-bucket policy |
...
- `config-signals.json` → `metrics.primary_sharpe_horizon_weeks`, `metrics.min_bucket_days`, `metrics.drop_partial_buckets`
