...
flowchart TB
    ...
    subgraph MAC["Weekly macro reports (provided)"]
...
**Macro reports are inputs, not outputs.** The three weekly markdown files (`analytics-weekly-summary.md`, `analytics-substitution-chain.md`, `analytics-rotation-targets.md`) are produced upstream by a separate agent system and are treated as fixed inputs to this pipeline.

**Week bundle convention.** Every weekly artifact — the three CSVs, the three macro reports, every step output — carries an `{iso_week}` tag in its filename or metadata. A "week bundle" is the complete set of files for a given ISO week. Live operation reads the current week's bundle; backtests load a historical week's bundle. Filename match is the only join key needed.
...
### 2.4 Portfolio positions — `positions.csv`

Currently held funds with values. Empty rows allowed if no holdings.

| Column | Type | Notes |
|---|---|---|
| isin | string | Foreign key to metadata |
| current_value_kr | number | Current market value in base currency (SEK) |
| acquired_at | date | When position was opened |
| cost_basis_kr | number | Original purchase value, for P&L calc |

### 2.5 Macro analytics (3 markdown files)

Provided as input — not generated by this pipeline. All three files cover the same ISO week.

| File | Purpose | Structure |
|---|---|---|
| `analytics-weekly-summary.md` | Themes by confidence level | YAML metadata block + sectioned narrative grouped by HIGH / MODERATE / DROPPED confidence |
| `analytics-substitution-chain.md` | Capital flows between asset classes | YAML metadata block + repeated `Fleeing / Toward / Mechanism` blocks |
| `analytics-rotation-targets.md` | High-conviction rotation themes | YAML metadata block + thematic targets with Strong/Moderate signal strength |

Each file begins with a YAML metadata block specifying `iso_week`, `period_start`, `period_end`, `generated_at`, and `run_id`. The pipeline must verify all three files share the same `iso_week` before processing.

### 2.6 Configuration file — `config.json`

All tunable parameters in one place. See [Section 3](#3-configuration-file).

---

## 3. Configuration file

A single JSON document that captures every tunable threshold, weight, and policy parameter. No agent should hardcode any of these — they all read from this file. The configuration version is recorded in every step output for replay.
...
```json
{
  ...
  "metrics": {
    "sharpe_horizons_weeks": [2, 12, 52],
    "primary_sharpe_horizon": 12,
    "min_bucket_days": 7,
    "drop_partial_buckets": true
  },
  ...
  "signal_rules": {
    ...
    "sell": {
      "any_of": [
        { "sharpe_2w_lt": 0 },
        { "current_drawdown_pct_lt": -1.5 },
        { "positive_windows_le": 1 }
      ]
    },
    "watch": {
      "missing_one_buy_criterion": true,
      "rotation_alignment_required": "Strong"
    }
  },
  ...
  "conviction_weights": {
    ...
    "thesis_validity": 0.20,
    "universe_context": 0.15
  },

  "cash_policy": {
    "floor_pct": 0.05,
    "macro_override_enabled": false,
    "macro_override_table": {
      "Crisis": 0.15,
      "Stagflation": 0.10,
      "Mixed": 0.07,
      "Risk-on": 0.05
  ...
  "portfolio_constraints": {
    ...
    "max_sector_pct": 0.30,
    "min_trade_kr": 5000,
    "skip_sell_below_conviction": 0.40,
    "skip_sell_below_conviction_unless_thesis_invalid": true
  },

  "macro_regime_enum": ["Risk-on", "Mixed", "Stagflation", "Crisis"],
  "thesis_validity_enum": ["Valid", "Partial", "Invalid", "NotApplicable"],
  "rotation_alignment_enum": ["Strong", "Partial", "None"]
}
```

The `macro_override_enabled: false` default reflects what we found in backtesting — raising the cash floor on macro signal lost money on the test universe because the regime tags fired after the drawdown rather than before. Keep the override mechanism in the config so it can be re-enabled when there's evidence to support it, but ship it off.
...
**What it does — example.** Reads metadata.csv (50 funds), summary.csv (1300+ buckets), positions.csv (8 held). Validates that every NAV bucket's ISIN exists in metadata. Marks funds whose ISIN appears in positions.csv as `currently_held: true` with their `current_value_kr`. Output: 50 fund records, of which 8 have `currently_held: true`.
...
- ISIN in positions.csv but not in metadata.csv → halt (held position must be known).

**Edge cases.**
- Empty positions.csv → all funds get `currently_held: false`.
...
**Role.** Read the three weekly markdown reports and extract structured macro context.
...
**Outputs.** Structured macro context:

```json
{
  "iso_week": "2026-W18",
  "macro_regime": "Stagflation",
  "macro_regime_secondary": "mixed risk-off",
  ...
  "catalysts": [
    {
      ...
      "intensity": "high",
      "weeks_active": 8,
      "affected_categories": ["Energy", "Branschfond, Energi", "Branschfond, Ny energi"]
    }
  ],
  "rotation_themes": [
    {
      "id": "rot_theme_energy_2026_W18",
      "label": "Integrated oil + inflation hedges",
      "signal_strength": "Strong",
      "matches_categories": ["Energy", "Branschfond, Ny energi"],
      "rationale": "Multiple chains point to capital moving into direct energy exposure..."
    }
  ]
}
```

**What it does — example.** Reads the substitution chain report. Identifies eight rotation chains, the first naming "Integrated oil producers" as the destination of capital fleeing oil-sensitive cyclicals due to Hormuz tensions. Reads the rotation-targets file, finds three Strong-signal themes. Reads the weekly summary, infers macro_regime = "Stagflation" from the "sticky inflation + cautious central banks + weakening growth" pattern. Cross-references catalysts against the fund universe categories from metadata.

**LLM prompt skeleton.** Provide the agent with: (1) the three markdown files verbatim, (2) the list of fund categories present in `metadata.csv`, (3) the controlled enum for `macro_regime`. Constrain output to the JSON schema. Prompt structure: "Extract a structured macro context from these weekly reports. The `macro_regime` must be one of {enum}. Catalysts must reference real categories from this list: {categories}. Do not invent themes that are not in the rotation-targets file."

**Error handling.**
- Invalid JSON in LLM response → retry once with corrective prompt ("Your previous response was not valid JSON, please re-emit"). Halt after second failure.
- `macro_regime` not in enum → halt with validation error.
- Catalyst references a category that does not exist in metadata → drop the catalyst with a warning.

**Edge cases.**
- The three input files have different `iso_week` values → halt; reports must be from the same week.
- Reports are unusually thin (few themes) → emit a low `regime_confidence` rather than fabricating signal.
- No rotation themes flagged Strong → emit empty `rotation_themes: []`. This is a valid state, not an error.
...
**Role.** Determine whether each fund's category aligns with active rotation themes.

**Execution type.** 🔀 Hybrid — code does dictionary matching, LLM is invoked only for ambiguous cases.

**Inputs.**
- Fund records (with `category`)
- MacroAnalyst output (with `rotation_themes`)
...
```json
{
  ...
  "matched_theme_id": "rot_theme_energy_2026_W18",
  "matched_theme_label": "Integrated oil + inflation hedges"
}
```

**What it does — example.** For Schroder ISF Global Energy (category: "Energy"): direct match to the energy rotation theme — `macro_alignment = Strong`. For Schroder ISF Greater China (category: "Kina & närliggande"): no theme matches → `macro_alignment = None`. For Schroder ISF QEP Glbl Qual (category: "Global, Mix bolag"): no theme directly matches but defensive equities theme is adjacent — invoke LLM to judge → `macro_alignment = Partial`.

**Code path.** Match `fund.metadata.category` against each `rotation_theme.matches_categories`. Direct match → `Strong`. No match → continue to LLM path or default to `None`.

**LLM path (only for unresolved cases).** Prompt: "Given fund category {category} and active rotation themes {themes}, is there partial alignment (defensive proximity, sector adjacency, factor overlap)?" Constrain output to enum.

**Error handling.**
- No rotation themes provided by MacroAnalyst → all funds get `macro_alignment: None`. This is valid.

**Edge cases.**
- A fund's category matches *multiple* themes → pick the highest-strength theme; if tied, the one with longest `matches_categories` list (more specific). Record only one match.

---

### 4.6 CatalystTagger

**Role.** Tag funds with active catalysts from the macro context.

**Execution type.** 🤖 LLM only.

**Inputs.**
- Fund records (with `category`, `name`)
- MacroAnalyst output (with `catalysts`)
...
```json
{
  "catalyst": {
    ...
    "intensity": "high",
    "weeks_active": 8,
    "match_reason": "Direct energy fund exposure"
  }
}
```

Or `"catalyst": null` for funds with no active catalyst.

**What it does — example.** Catalyst list contains "Hormuz disruption" affecting Energy categories. Schroder ISF Global Energy is tagged with the catalyst. Schroder ISF Glbl Alt Engy is tagged with the same catalyst (secondary energy exposure). Schroder ISF Em Mkts is *not* tagged because emerging markets is not in the catalyst's affected_categories.

**LLM prompt skeleton.** "For each fund {name, category}, determine whether the catalyst {catalyst} applies as Direct, Indirect, or Not applicable. Use only the affected_categories listed for the catalyst plus reasonable category-name similarity. Do not invent fund-specific exposure."

**Error handling.**
- No catalysts in macro context → all funds get `catalyst: null`. Valid.
- LLM emits a fund-catalyst mapping that contradicts `affected_categories` → log warning, drop the spurious tag.

**Edge cases.**
- Multiple catalysts apply to a single fund → emit only the most directly applicable one, with `match_reason` explaining why.
...
- `analytics-substitution-chain.md` (for rotation pairing reasoning)
...
**Conviction scoring.** A weighted combination of five components, each scored 0 to 1 and combined per the weights in `config.conviction_weights`:
...
**Rotation pairing.** When a `*Exit` and an `*Entry` share the same `matched_theme_id` from MacroAligner, link them with a shared `rotation_pair_id` of the form `rot_{iso_week}_{letter}`. The substitution chain markdown is consulted only for naming and rationale support, not for pair detection.

**What it does — example.** Three buys in current run (Em Mkts, Glb Em Mkt Opps, QEP Glbl Qual). Conviction scores 0.72 / 0.81 / 0.65 → ranked 2 / 1 / 3. For each, find peers in same category, generate differentiators. Global Energy SELL has matched_theme_id "rot_theme_energy_2026_W18"; Glbl Alt Engy BUY has the same matched_theme_id. They are paired with `rotation_pair_id = rot_2026_W18_a`.
...
| Value | Meaning |
|---|---|
| Risk-on | Broadly positive, growth-supportive |
| Mixed | No clear directional bias |
| Stagflation | Sticky inflation + cautious central banks + slowing growth |
| Crisis | Severe drawdown, broad asset stress |
...
| Value | Meaning |
|---|---|
| Strong | Direct match to a Strong-signal rotation theme |
| Partial | Adjacent / sector-overlap match |
| None | No theme matches |
...
- Keep a `config.json` snapshot in every step output (`config_version` field).
...
**Observed in.** Backtesting the macro override (raise floor to 10–15% on Stagflation/Crisis) over the 2025–2026 Schroder year produced 16 regime flips, with macro tags often firing *after* drawdowns rather than before. Net effect: 1.3–2.1% drag against a 32.7% portfolio return.
...
**Macro regime.** One of {Risk-on, Mixed, Stagflation, Crisis}. Extracted by MacroAnalyst from the weekly reports.
