...
public sealed class Step7ThesisValidatorViewModel : StepViewModel
{
    ...
    {
        // 8c: prefer SQLite Step07Json columns; disk fallback for per-step button flow.
        if (_isinProgress is not null)
        {
            var sqliteResult = await IsinProgressOutputLoader.LoadStepFundsAsync(
                _isinProgress, RunId, row => row.Step07Json);
            if (sqliteResult is not null)
            {
                OutputJson = sqliteResult.Json;
                OutputSummaryText = $"{sqliteResult.Funds.Count} funds — theses validated";
                return;
            }
        }
