...
/// (<c>YieldRaccoon.Infrastructure.Services.FundSnapshotCsvExportService</c>). It lives beside the
/// copied read-only ORM model in this folder because it is upstream code, not FikaFinans code:
/// it talks to YR's schema and emits YR's snapshot CSV — the very file
/// <c>Pipeline.Csv.SnapshotCsvParser</c> consumes. Keep it in sync with the original rather than
/// evolving it here; divergence means our numbers stop matching YR's own exports.
/// </para>
/// <para>
/// <b>Deviations from upstream:</b> <see cref="TakeHorizonSlice"/> and the two horizon constants
/// are <c>internal</c> rather than <c>private</c> so the per-ISIN fetch seam can reuse the horizon
/// windowing instead of re-deriving it, and the awaits carry <c>ConfigureAwait(false)</c> as
/// library code should. Neither changes a number. Unlike its sibling
/// <see cref="FundStatisticsCsvExportService"/> this one has no in-memory <c>ComputeAsync</c>
/// overload — the snapshot seam calls the calculator directly, so there was nothing to add.
/// </para>
/// </remarks>
public class FundSnapshotCsvExportService : IFundSnapshotCsvExportService
{
    ...
    internal const int Horizon12wDays = 84;
    internal const int Horizon1yDays = 365;
        ...
        IProgress<(int processed, int total)>? progress = null)
    {
        ...
        if (asOfDate is null)
        {
            ...
            await WriteHeaderOnlyAsync(csvOutputPath).ConfigureAwait(false);
            return 0;
        }

        var fundProfiles = await FundQueryHelpers
            .ReadFundProfilesAsync(connection, companyName, minNumberOfOwners)
        ...
        foreach (var (isin, _) in fundProfiles)
        {
            var navSeries = await FundQueryHelpers
                .ReadNavSeriesAsync(connection, isin, earliestNeeded)
                .ConfigureAwait(false);
        ...
        await WriteCsvAsync(csvOutputPath, snapshots).ConfigureAwait(false);
    ...
    /// </summary>
    internal static IReadOnlyList<(DateOnly date, decimal nav)> TakeHorizonSlice(
