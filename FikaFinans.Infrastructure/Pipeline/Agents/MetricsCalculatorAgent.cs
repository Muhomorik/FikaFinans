...
public sealed class MetricsCalculatorAgent : IMetricsCalculatorAgent
{
    ...
    public MetricsCalculatorAgent(IPathsService paths)
    {
    ...
    public DataLoaderOutput Run(string isoWeek, string runId)
    {
        ...
        var outputPath = _paths.MetricsCalculatorOutput(isoWeek, runId);
        ...
        var config = File.Exists(configPath)
            ? JsonSerializer.Deserialize<MetricsCalculatorConfig>(
                File.ReadAllText(configPath), JsonOptions.Default)
              ?? throw new InvalidDataException($"Failed to deserialize config at {configPath}")
