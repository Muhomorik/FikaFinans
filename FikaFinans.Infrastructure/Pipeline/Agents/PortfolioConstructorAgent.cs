...
public sealed class PortfolioConstructorAgent : IPortfolioConstructorAgent
{
    ...
    public PortfolioConstructorAgent(IPathsService paths) : this(paths, PortfolioConstructorConfig.Default) { }

    public PortfolioConstructorAgent(IPathsService paths, PortfolioConstructorConfig config)
    {
        ...
        List<ConstraintViolation> violations)
    {
        ...
        var thesis     = fund.ThesisValidity ?? ThesisValidity.NotApplicable;
        ...
        List<ConstraintViolation> violations)
    {
        ...
        if (totalDeployable <= 0m)
        {
            ...
            {
                var idx = trades.FindIndex(t => t.Isin == isin);
                if (idx < 0) continue;
                var t = trades[idx];
                rejected.Add(new RejectedRecommendation
                {
        ...
        var headroomByIsin = buys.ToDictionary(
            ...
            isin =>
            {
                var f = funds.First(x => x.Isin == isin);
        ...
        if (_config.Sizing.DropBuyIfFallsBelowMinTrade)
        {
            ...
            while (true)
            {
                var smallest = amounts
                    ...
                    .Select(kv => ((Isin Isin, decimal Amount)?) (kv.Key, kv.Value))
                    .FirstOrDefault();
                if (smallest is null) break;

                var dropIdx = trades.FindIndex(t => t.Isin == smallest.Value.Isin);
                var dropped = trades[dropIdx];
                var capped  = headroomByIsin[smallest.Value.Isin] < requested * scaling;
                var because = capped
                    ? $"min_trade_dropout (capped at {smallest.Value.Amount.ToString("F0", CultureInfo.InvariantCulture)} kr by concentration headroom; below min_trade_kr {minTrade.ToString("F0", CultureInfo.InvariantCulture)})"
                    : $"min_trade_dropout (scaled to {smallest.Value.Amount.ToString("F0", CultureInfo.InvariantCulture)} kr < {minTrade.ToString("F0", CultureInfo.InvariantCulture)} kr)";
                rejected.Add(new RejectedRecommendation
                {
                    Isin                 = dropped.Isin,
                    SourceRecommendation = dropped.SourceRecommendation,
                    RejectedBecause      = because,
                    WouldHaveBeen        = $"{dropped.TradeType} {smallest.Value.Amount.ToString("F0", CultureInfo.InvariantCulture)} kr",
                });
                violations.Add(new ConstraintViolation
                {
                    ViolationType = "min_trade_dropout",
                    Isin          = dropped.Isin,
                    Value         = smallest.Value.Amount.ToString("F2", CultureInfo.InvariantCulture),
                    Message       = $"{dropped.Isin} dropped — final amount {smallest.Value.Amount.ToString("F0", CultureInfo.InvariantCulture)} kr below min_trade_kr.",
                });
                trades.RemoveAt(dropIdx);
                amounts.Remove(smallest.Value.Isin);
                headroomByIsin.Remove(smallest.Value.Isin);

                if (amounts.Count == 0) break;
                var bonus = smallest.Value.Amount / amounts.Count;
        ...
        List<ConstraintViolation> violations)
    {
        ...
        {
            var trade = trades[i];
            var fund  = funds.FirstOrDefault(f => f.Isin == trade.Isin);
        ...
        List<ConstraintViolation> violations)
    {
        ...
        while (true)
        {
            ...
            // contract clause.
            var newBuys = members
                .Select(m => (Member: m, Trade: trades.FirstOrDefault(t => t.Isin == m.Isin)))
            ...
            var fund       = funds.First(f => f.Isin == largest.Isin);
            var trimAmount = Math.Round(Math.Min(excess, largest.Value), 2, MidpointRounding.AwayFromZero);
            var conviction = fund.ConvictionScore ?? 0m;

            var idx  = trades.FindIndex(t => t.Isin == largest.Isin);
    ...
    private sealed record PostTradePosition(Isin Isin, string Category, decimal Value, FundLayer Layer);
        ...
        List<Trade> trades, IReadOnlyList<FundRecord> funds)
    {
        ...
        {
            var trade   = trades.FirstOrDefault(t => t.Isin == fund.Isin);
            var current = fund.CurrentValueKr ?? 0m;
            var post    = current;
        ...
        foreach (var pair in executedPairs)
        {
            ...
            if (hasSell && !hasBuy)
            {
                var rejectedBuy = rejected.FirstOrDefault(r =>
                    ...
                    string.Equals(
                        funds.FirstOrDefault(f => f.Isin == r.Isin)?.RotationPairId,
                        pair.Key,
