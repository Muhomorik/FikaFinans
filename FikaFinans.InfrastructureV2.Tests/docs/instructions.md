...
> 4. `inputs/summary-csv-plan.md`, `inputs/snapshot-csv-plan.md`, `inputs/FUND-STATISTICS-EXPORT-AGENT-GUIDE.md` — fund CSV schemas.
> 5. `inputs/analytics-json-schema.md` — the macro analytics JSON schema (camelCase, GUID-keyed, ISO 8601 dates).
> 6. `inputs/portfolio_structure.md` — layer definitions (`core`, `writeoff`) and pinned-fund policy.
> 7. The `inputs/` folder — real sample data for testing.
> 8. The `examples/` folder — reference outputs from a prior chain run.
>
...
│
├── inputs/                               ← real sample data + input schemas
...
│   ├── analytics-json-schema.md                ← schema spec for the 3 analytics JSONs
│   ├── portfolio_structure.md                  ← layer definitions + pinned funds
...
│   ├── positions.csv                           ← currently held funds (incl. Cash row)
...
**Input data** (`inputs/`) is one ISO-week bundle from a real fund family. Use it to write integration tests against the full pipeline. The three macro analytics JSON files (`analytics-weekly-summary.json`, `analytics-substitution-chain.json`, `analytics-rotation-targets.json`) are the upstream input to MacroAnalyst (step 03) — see `inputs/analytics-json-schema.md` for their schema. The three CSVs and `positions.csv` are inputs to DataLoader (step 01); `portfolio_structure.md` is also read by DataLoader to tag funds with their layer (`core` / `writeoff` / `active`).
...
| **`{iso_week}` (CSVs) and `periodIsoWeek` (analytics JSONs) must all match** | Avoids stale-data bugs; pipeline halts on mismatch before any agent runs |
...
| What does each CSV column mean? | `inputs/FUND-STATISTICS-EXPORT-AGENT-GUIDE.md` |
| Why is `sharpe_2w` sometimes `null`? | `inputs/summary-csv-plan.md` §7.3 |
| How do I backtest? | Load the historical week's bundle from `inputs/{iso_week}/` and re-run from step 01 |
| What's the schema of the analytics JSON inputs? | `inputs/analytics-json-schema.md` |
| What does `core` / `writeoff` / `active` layer mean? | `inputs/portfolio_structure.md` + `01-dataloader.md` |
| How is the `Cash` row in `positions.csv` handled? | `01-dataloader.md` (carved into `cash_available_kr`) |
...
| Layer | Status |
|---|---|
| Data input specs | ✅ done (analytics inputs migrated to JSON — see `inputs/analytics-json-schema.md`) |
| Pipeline architecture | ✅ done |
| Agent contracts (10) | 🟡 detailed contracts exist for 01–10. **03 migrated to JSON inputs.** **01 + 10 wired for layers (`core`/`writeoff`) and Cash row.** **05, 06, 09 still need a refresh pass** to align with the new analytics JSON schema (enum changes, `matches_categories` removed) |
