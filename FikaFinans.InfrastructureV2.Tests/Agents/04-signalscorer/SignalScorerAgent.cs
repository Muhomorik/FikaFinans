...
public sealed class SignalScorerAgent
{
    public DataLoaderOutput Run(string isoWeek, string runId)
    {
        var inputPath  = Paths.MetricsCalculatorOutput(isoWeek, runId);
        var configPath = Paths.Config04SignalsJsonAbs;
        var outputPath = Paths.SignalScorerOutput(isoWeek, runId);
        ...
        var config = File.Exists(configPath)
            ? SignalScorerConfig.Load(configPath)
