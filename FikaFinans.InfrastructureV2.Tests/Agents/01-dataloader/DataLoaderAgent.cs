...
public sealed class DataLoaderAgent
{
    ...
    {
        var metadataPath  = Paths.MetadataCsv(family, isoWeek);
        var summaryPath   = Paths.SummaryCsv(family, isoWeek);
        var snapshotPath  = Paths.SnapshotCsv(family, isoWeek);
        var positionsPath = Paths.PositionsCsvAbs;
        var structurePath = Paths.PortfolioStructureMdAbs;
        ...
        try
        {
            ...
            WriteJson(Paths.DataLoaderOutput(isoWeek, runId), output);
            return output;
        }
        catch (DataLoaderHaltException halt)
        {
            WriteErrorFile(Paths.DataLoaderError(isoWeek, runId), halt, family, isoWeek, runId);
            throw;
        }
    }
    ...
    private static DataLoaderOutput Join(
        ...
        IReadOnlyDictionary<string, IReadOnlyList<NavBucket>> summary,
        IReadOnlyDictionary<string, FundSnapshot> snapshots,
        PositionsParseResult positions,
        PortfolioStructure structure)
    {
        var warnings = new List<string>(positions.Warnings);
        var metaByIsin = metadata.ToDictionary(m => m.Isin, StringComparer.Ordinal);
        var positionByIsin = positions.Holdings.ToDictionary(p => p.Isin, StringComparer.Ordinal);
        ...
        {
            var matchesByIsin = p.Isin != null && metaByIsin.ContainsKey(p.Isin);
            var matchesByName = p.Isin == null && p.Name != null && metaNames.Contains(p.Name);
            if (!matchesByIsin && !matchesByName)
            {
                var label = p.Isin ?? p.Name ?? "<empty>";
